...
from config.hypersync_client import Chain
...
class TxnByIdRequest(BaseModel):
    ...
    chain: Optional[Chain] = Field(Chain.ETHEREUM, description="Blockchain to query", example="ethereum")
...
async def txn_by_id_route(request: TxnByIdRequest):
    return await txn_by_id(tx_hash=request.tx_hash, chain=request.chain)
...
async def blocks_transactions_hashes_route(request: BlocksTransactionsHashesRequest):
    return await blocks_transactions_hashes(from_block=request.from_block, to_block=request.to_block, chain=request.chain)
...
async def chain_info_route(chain: Optional[Chain] = Chain.ETHEREUM):
    return await chain_info(chain=chain)
